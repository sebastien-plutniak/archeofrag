...
Whereas most objects have clear boundaries, this is not the case for layers: the boundary between two layers is a fuzzy one. 
...
2. determine the topological properties of the relationships between fragments within a given layer.
...
frag.graph.plot(simul.g, "layer", asp=F)
```
...
frag.graph.plot(abu.g12, "layer", asp=F )
```
