test_that("admixture for 2 layers", {
  ...
  expect_equal(round(as.numeric(frag.layers.admixture(g, "layer")), 6), 0.0553)
})
...
test_that("admixture for 3 layers", {
  ...
  expect_equal(round(as.numeric(frag.layers.admixture(g, "layer")), 6), 
               expected = c(0.0398, 0.2067, 0.0203))
})
...
test_that("parameters are correctly transmitter to frag.edges.weighting", {
  ...
  expect_identical( c(frag.layers.admixture(g1, "layer"), use.names = F),
                    expected = c(frag.layers.admixture(g, "layer", "morpho", "x", "y", "z")[1], use.names = F))
})
